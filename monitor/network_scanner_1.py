    ...
    while not queue.empty():
        ...
        for current_interface in get_interfaces_via_ssh():
            if current_interface["ip"] not in scanned_ips and current_interface["interface"] != "lo":
                scanned_ips.add(str(current_interface["ip"]))
                add_to_db_interface(
        ...
        #TODO пересмотреть способ добавления соседей/связей.
        # Возможно, стоит создать отдельную таблицу чисто с названиями устройств (без сетевых адаптеров)

        for neighbor in neighbors:
            if neighbor["ip"] not in scanned_ips:
                # Получаем хостнейм соседа через SSH
                try:
                    connect_via_ssh(neighbor["ip"])
                    neighbor_hostname = get_hostname_via_ssh()
                except Exception as e:
                    print(f"⚠️ Ошибка подключения к соседу {neighbor['ip']}: {e}")
                    continue

                # Добавляем соседа в очередь для дальнейшего сканирования
                queue.put(neighbor["ip"])

                # Добавляем устройство в базу данных с полученным хостнеймом
                add_to_db_device(neighbor_hostname, device_type)

                # Добавляем интерфейс соседа в базу данных
                add_to_db_interface(neighbor["ip"], neighbor["mac"], neighbor_hostname, neighbor["interface"])

                # Добавляем связь между текущим устройством и соседом
                add_to_db_connection(current_hostname, neighbor["ip"], neighbor["mac"])
